...
1.  **What is the author defining in lines 15-17? Where else in the code do these defined variables show up? What advantage(s) is there to defining these values here, as variables, rather than defining the values directly throughout the script?**
    ...
    -   This is defining a vector of hex codes to be used later in the code. 'bg' and 'accent' pull out the specific hex codes from the defined vector so they can be called independently. These are colors to be used across graphics further in the code. Defining these now is useful as they have intuitive names that clarify where they should be applied, and also this means that no copy and pasting of stylistic elements needs to be done later down the line to get these colors.
...
2.  **In your own words, explain what the function, `font_add_google()`, does. What's the difference between the two arguments, `name` and `family`?**
    ...
    -   This function pulls fonts from the Google Fonts Repository. The name itself is the one listed by Google Fonts, the family parameter is what R uses as a function when plotting. 
...
17. **Why does the author render `plot.caption` using `ggtext::element_markdown()`, rather than `element_text()` (like he does for rendering `plot.title` and `text`)?**
    ...
    -   `element_markdown()` specifically allows parsing of code formatting. Printing out `quote1`-`quote3`, you can see that these strings have line break formatting which is important to how they will print out on the final visual, so `element_markdown()` allows for stylizing that `plot.caption` would not.
...
19. **Can you think of one reason the author may have chosen to separate the construction of `plot_base` and `plot_final`?**
    ...
    -   Other than the way this workflow logically layers on elements to the plot base to create a final visual, it would be easy to reuse it for another visual if you wanted to seperate them out differently.
...
20. **During week 2, we discuss [Choosing the right graphic form](https://eds-240-data-viz.github.io/course-materials/lecture-slides/lecture2.1-choosing-graphic-forms-slides.html#/title-slide). Refer to this lecture when answering the sub-questions, below:**
    ...
    a.  **What "perceptual tasks" (from Cleveland & McGill's heirarchy) must the viewer perform to extract information from these visualizations?**
        ...
        -   Viewers primarily have to interpret length, shading, and color saturation.
    ...
    b.  **What task(s) do you think the author wanted to enable or message(s) he wanted to convey with these visualizations (see lecture 2.1, slide 16 for examples)? Be sure to note at least one task / message for each of the three data viz.**
        ...
        -   The primary objective of the author was to envision temporal and spatial patterns and patterns in observations.
    ...
    c.  **Name at least one caveat to the "hierarchy of perceptual tasks" that the author employed to achieve a goal(s) you noted in question b?**
        ...
        -   This visual highly relies on shading to convey its message, but this is effective thematically to show the frequency of that grouping, and particularly because the most commonly observed 'ufo' is light events.

21. **Describe two elements of this piece that you find visually-pleasing / easy to understand / intuitive. Why?**

    -   I love how the geom_faceting lays out state information in the general shape of the U.S. because it makes it very easy to see spatial patterns over a large area in a visual search that most people are somewhat familiar with already.

22. **Describe two elements of this piece that you feel could be better presented in a different way. Why?**

    -   This is fairly subjective but I think the quotes aren't necessarily informative with all the space they are taking up. I feel like there could be a seperate visual element like a word cloud to better represent trends in the notes part of the data. Also at a certain opacity it is pretty hard to see some of the plots, so I think it would be beneficial to increase the opacity range just a little.
...
23. **Describe two new things that you learned by interpreting / annotating this code. These could be packages, functions, or even code organizational approaches that you hadn't previously known about or considered.**
    ...
    -   I learned about the {geo_facet} package which could be very useful for any type of spatial data representation. I had also never seen {glue} be used to present markdown text visually before.
...
24. **How, if at all, did you use AI tools to help you interpret this code? Describe your approach to using these tools for this assignment. In what ways was consulting the documentation more (or less) helpful than using AI?**
    ...
    -   If I could not figure out a function or chunk myself by testing it, and couldn't to find straightforward answers within the R documentation, Stack Overflow, or other internet resources, AI was useful for interpretting larger chunks of code together to piece together a better overall picture of why choices were made. I specifically used an LLM to learn more about how `element_markdown()` would work because the documentation wasn't super clear and it only made sense with the context of having tried to print out `quote#` and understanding that there was format evaluation going on under the hood.
